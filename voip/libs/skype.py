        ...
        try:
            ...
            if result.invoice_id != '':
                ...
                if result.has_pay_info():
                    self.bot.send_group_message(group_id, 'or you can transfer: %s %s to %s via %s in 20 minutes (otherwiise generate new one).' % (result.amount, result.pay_currency, result.pay_address, result.get_network_display_name()))
            else:
